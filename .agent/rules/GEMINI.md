...
# GEMINI.md - Layer 1: Core Protocol

> **Cross-Platform AI Configuration** - Mirrored with CLAUDE.md  
> **Architecture**: 3-Layer Separation of Concerns  
> **Platform**: Gemini AI (Antigravity, Flash/Pro)
...
| **COMPLEX CODE**| "build", "create", "implement", "refactor" | Layer 1 + Layer 2 (full) + Agent | **{task-slug}.md Required** |
| **DESIGN/UI** | "design", "UI", "page", "dashboard" | Layer 1 + Layer 2 + Agent | **{task-slug}.md Required** |
...
### 🏗️ Core Development Standards (MANDATORY)

**Classify every request against these 5 Pillars:**

1.  **Data-First Protocol**: You MUST define the Input/Output JSON contract (Schema/OpenAPI) before writing any logic. **No schema = No code.**
2.  **Standardized Responses**: Use strict JSON envelopes (`data`, `error`, `meta`) and Design System tokens.
3.  **Spec-Driven Development**: `PLAN.md` is the *Functional Specification*. If code diverges, update the plan first.
4.  **Automation First**: Prioritize Webhooks, Cron, and Background Workers over synchronous loops.
5.  **Strict Definition of Done**: A task is ONLY complete when: Code is written, Tests pass, Docs are updated, and Manual Verification is confirmed.
...
- **Infrastructure & Safety Mandate:** Every agent is responsible for the deployability and operational safety of their changes. Follow the "5-Phase Deployment Process" (Prepare, Backup, Deploy, Verify, Confirm/Rollback). Always verify environment variables and secrets security. **Never modify .env files or database data directly; instruct the user to make these changes or use migrations/scripts.**
...
## LAYER 2: CODE RULES (When Writing Code)
...
|------------|---------|---------|
| **Manual Audit** | `python .tmp/checklist.py .` | Priority-based project audit |
| **Pre-Deploy** | `python .tmp/checklist.py . --url <URL>` | Full Suite + Performance + E2E |
...
> 🔴 **Agents & Skills can invoke ANY script** via `python .agent/skills/<skill>/scripts/<script>.py` or `.tmp/checklist.py`
...
## LAYER 3: DESIGN RULES (Reference)
...
**Path Awareness:**
- **Layer 1 (Core)**: `.agent/rules/GEMINI.md` (this file)
...
This configuration is **mirrored** with CLAUDE.md to ensure consistent behavior across AI platforms:

- **Gemini AI**: Uses this file (`.agent/rules/GEMINI.md`)
- **Claude AI**: Uses `.agent/rules/CLAUDE.md` (identical Layer 1 rules)
